...
def featureExtraction():
  ...
  text.insert(END,"Extracted Features : "+str(total - X_train1.shape[1])+"\n\n")
...
def predictPerformance():
  ...
  for i in range(len(test)):
    if str(y_pred[i]) == '0':
      text.insert(END,"X=%s,\n Predicted=%s" % (X_test[i], ' Reason of Poor Performance : Dropout\n')+" Extracted Feature : "+str(y_pred[i])+"\n")
    elif str(y_pred[i]) == '1':
      text.insert(END,"X=%s,\n Predicted=%s" % (X_test[i], ' Reason of Poor Performance : Failing Subject \n')+" Extracted Feature : "+str(y_pred[i])+"\n")
    elif str(y_pred[i]) == '2':
      text.insert(END,"X=%s,\n Predicted=%s" % (X_test[i], 'Reason of Poor Performance : Failing Subject \n')+" Extracted Feature : "+str(y_pred[i])+"\n")
    elif str(y_pred[i]) == '3':
      text.insert(END,"X=%s,\n Predicted=%s" % (X_test[i], 'Good Performance\n')+" Extracted Feature: "+str(y_pred[i])+"\n")
    elif str(y_pred[i]) == '4':
      text.insert(END,"X=%s,\n Predicted=%s" % (X_test[i], 'Good Performance\n')+" Extracted Feature : "+str(y_pred[i])+"\n")                                           


def graph():
    height = [svm_acc,decision_acc, boosting_acc]
    bars = ('SVM','Decision Tree','Gradient Boosting')
